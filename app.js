...
G.generateLevel=function(lesson,lvl){
  ...
  // Solid ground everywhere — no gaps
  const platforms=[{x:0,y:groundY,w:totalW,h:40,type:'ground'}];
  ...
  for(let i=0;i<Math.min(qWords.length,4+lvl);i++){
    const qx=segW*(i+1)*0.8+100;
    const qy=groundY-145-(i%2)*12;
    qBlocks.push({x:qx,y:qy,w:40,h:40,word:qWords[i%qWords.length],answered:false});
    platforms.push({x:qx-34,y:qy+96,w:108,h:14,type:'float',helper:true});
  }
  ...
  return{platforms,qBlocks,coins,enemies,gate:{x:totalW-200,y:groundY-80,w:50,h:80,opened:false},totalW,groundY};
};
...
G.initEngine=function(level,lesson){
  ...
  E.platforms=level.platforms;E.gaps=[];E.qBlocks=level.qBlocks;E.coins=level.coins;E.enemies=level.enemies;E.gates=[level.gate];
